...
if not control_rows.empty:
    ...
    coverage_row = coverage_df[
        coverage_df["Document ID"]
        == DOCUMENT_ID
    ]

    if not coverage_row.empty:

        document_title = coverage_row.iloc[0][
            "Document Title"
        ]
...
"""
    ...
    control_text = """
No Annex A controls are directly mapped
to this document.

Generate the document using:

- ISO Clause Requirements
- Coverage Matrix Mapping
- Reference Templates

Do not introduce unrelated controls.
"""
...
7. Follow the style, tone, and structure of the templates.

8. Create comprehensive, professional, audit-ready content.
...
Do not provide short bullet-point answers unless necessary.

For major sections provide:
- Multiple paragraphs
- Detailed requirements
- Practical implementation guidance
- Governance expectations

Write at the level expected from a Big 4 consulting deliverable or ISO 27001 certification-ready policy.

Assume the document will be reviewed by:
- External auditors
- Certification bodies
- Clients
- Senior management

Expand every section sufficiently.
Avoid generic statements.
Provide specific and actionable policy requirements.

9. Every supplied clause and control must be addressed.
9A.For each ISO clause and control:
- Explain the requirement.
- Explain organizational expectations.
- Explain responsibilities.
- Explain governance requirements.
- Explain monitoring requirements where relevant.

Do not merely restate the control title.
